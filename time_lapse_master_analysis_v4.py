...
def plot_averaged_timeseries(raw_list, output_dir):
    save_dir = os.path.join(output_dir, "TimeSeries_Averaged")
    os.makedirs(save_dir, exist_ok=True)
    
    materials = sorted(set(r["material"] for r in raw_list))
    cmap = plt.cm.viridis
    colors = dict(zip(TIME_POINTS, cmap(np.linspace(0.15, 0.85, len(TIME_POINTS)))))
    time_bins = np.arange(0, HARD_CUTOFF_MIN + 1, 1)

    # Define the two metrics we want to loop through and plot
    metrics_to_plot = [
        ("Voltage", "voltage_data", "Voltage (V)"),
        ("Angle", "angle_data", "Angle (deg)")
    ]

    for mat in materials:
        for metric_name, data_key, ylabel in metrics_to_plot:
            fig, ax = plt.subplots(figsize=(8, 6))

            for tp in TIME_POINTS:
                # Filter for this material, timepoint, and ensure the specific data exists
                trials = [r for r in raw_list if r["material"] == mat and r["time_point"] == tp and r[data_key] is not None]
                if not trials:
                    continue
                ...
                for tr in trials:
                    ...
                    for i in range(len(time_bins)-1):
                        m = (t >= time_bins[i]) & (t < time_bins[i+1])
                        vals.append(np.nan if not np.any(m) else np.mean(v[m]))
                    binned.append(vals)

                # Use catch_warnings to hide the annoying "Mean of empty slice" prints
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", category=RuntimeWarning)
                    binned = np.array(binned)
                    mean = np.nanmean(binned, axis=0)
                    std = np.nanstd(binned, axis=0)
                    
                x = time_bins[:-1] + 0.5

                ax.plot(x, mean, lw=3, color=colors[tp], label=TITLE_MAP.get(tp, tp))
                ax.fill_between(x, mean-std, mean+std, color=colors[tp], alpha=0.25)

            apply_academic_axes(ax, "Time (min)", ylabel)
            ax.set_xlim(0, HARD_CUTOFF_MIN)
            
            # Clean up the bottom bounds for Angle ONLY, so we don't chop off negative voltages
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, f"{mat}_{metric_name}_TimeSeries_Averaged.png"), dpi=300)
            plt.close()
...
if __name__ == "__main__":
    ...
    print("Generating Averaged Time Series...")
    plot_averaged_timeseries(raw_list, output_dir)
