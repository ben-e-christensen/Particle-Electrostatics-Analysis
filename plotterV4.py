...
# If you *know* your commanded RPM range, you can hard-limit here (optional):
VALID_RPM_MIN = 1
VALID_RPM_MAX = 1000  # set to 30 or 26 if you want (e.g., 30)

# ======================================================

def parse_duration_minutes(dur_str: str, default: int = 60) -> int:
    """Accepts strings like '60mins', '12min', '12', '12mins_run' -> returns int minutes."""
    if not dur_str:
        return default
...
def infer_commanded_speeds(df: pd.DataFrame) -> list[int]:
    """
    Infer the commanded RPM set from a trial's motor_speed time-series by finding
    the most frequent rounded speeds and filtering out transient/rare values.
    """
    if df.empty or "motor_speed" not in df.columns:
        return []

    rounded = df["motor_speed"].round(0).astype("Int64")
    vc = rounded.value_counts(dropna=True)

    # filter to positive and within any optional bounds
    vc = vc[vc.index.notna()]
    vc = vc[(vc.index.astype(int) >= VALID_RPM_MIN) & (vc.index.astype(int) <= VALID_RPM_MAX)]

    if vc.empty:
        return []

    # Count threshold to avoid including brief transients like -3, 0, etc.
    # Use a small fraction of samples OR a minimum absolute count.
    min_count = max(200, int(0.005 * len(df)))
    allowed = sorted(int(k) for k, cnt in vc.items() if cnt >= min_count)

    # Fallback: if filtering was too strict, just use all positive rounded uniques
    if not allowed:
        allowed = sorted(int(x) for x in rounded.dropna().unique() if int(x) >= VALID_RPM_MIN)

    return allowed

def snap_to_allowed_speeds(values: pd.Series, allowed: list[int]) -> pd.Series:
    """
    Snap numeric values to the nearest allowed speed.
    Returns Int64 series. If allowed is empty, falls back to rounding.
    """
    if allowed is None or len(allowed) == 0:
        return values.round(SPEED_ROUNDING_PRECISION).astype("Int64")

    allowed_arr = np.array(sorted(allowed), dtype=float)
    ...
    for x in v:
        if np.isnan(x):
            ...
            continue
        idx = int(np.argmin(np.abs(allowed_arr - x)))
        snapped.append(int(allowed_arr[idx]))
    return pd.Series(snapped, index=values.index, dtype="Int64")
...
def generate_temporal_speed_grids(df, speeds, materials, metadata, output_dir):
    ...
    for speed in speeds:
        ...
        fig.suptitle(
            f"Speed: {speed} RPM | Time-Colored Drift\nVol: {metadata['vol']} | {metadata['cond']}",
            fontsize=16
        )
        ...
        for j, material in enumerate(materials):
            ...
            sc = ax.scatter(
                x, y,
                c=mat_speed_df["minute_bin"],
                cmap="coolwarm",
                vmin=1, vmax=time_max,
                alpha=0.8, s=60,
                edgecolors="black"
            )
        ...
        if sc is not None:
            ...
            fig.colorbar(sc, cax=cbar_ax, label=f"Time (min, fixed 1–{time_max})")
...
def process_and_plot_single_run(top_dir, metadata):
    ...
    for material_folder in material_folders:
        ...
        for trial_folder in trial_folders:
            ...
            if len(df.columns) == 15:
                ...
                df.columns = cols_15 + ["ch3_flag"]

            if df.empty:
                continue

            # Infer commanded speeds for this trial (used to snap minute-aggregated speeds)
            commanded_speeds = infer_commanded_speeds(df)

            # 1) Tag Directions strictly to prevent double points
            peak_speed_val = df["motor_speed"].max()
            peak_block = df[df["motor_speed"] == peak_speed_val]
            if len(peak_block) == 0:
                continue

            mid_peak_idx = peak_block.index[len(peak_block) // 2]
            df["raw_direction"] = "Increasing"
            df.loc[mid_peak_idx:, "raw_direction"] = "Decreasing"

            # 2) Assign Time Bins (start at 1 instead of 0)
            t0 = df["timestamp"].iloc[0]
            df["minute_bin"] = ((df["timestamp"] - t0) / 60).astype(int) + 1

            # 3) Ensure each minute has only ONE direction (the majority direction)
            direction_map = (
                df.groupby("minute_bin")["raw_direction"]
                .agg(lambda x: x.value_counts().index[0])
                .to_dict()
            )
            df["direction"] = df["minute_bin"].map(direction_map)

            # 4) Avalanche Detection (peaks)
            ...
            # 5) Aggregate per minute
            ch_min = df.groupby(["minute_bin", "direction"]).agg(
                voltage_std=("CH2_volts", "std"),
                motor_speed=("motor_speed", "median"),  # robust against transient spikes
            ).reset_index()
            ...
            # Fallback angle per minute if no peaks were detected that minute
            raw_ang_min = df.groupby(["minute_bin", "direction"]).agg(
                angle_fallback=("ellipse_angle_deg", "max")  # choose max or mean
            ).reset_index()

            minute_data = pd.merge(ch_min, ang_min, on=["minute_bin", "direction"], how="left")
            minute_data = pd.merge(minute_data, raw_ang_min, on=["minute_bin", "direction"], how="left")

            minute_data["angle_mean"] = minute_data["angle_mean"].fillna(minute_data["angle_fallback"])
            minute_data = minute_data.drop(columns=["angle_fallback"])

            # Snap minute-level speeds to nearest commanded speed (removes weird -3 RPM, drift bins, etc.)
            minute_data["grouped_speed"] = snap_to_allowed_speeds(minute_data["motor_speed"], commanded_speeds)

            # Drop any non-physical / out-of-range speeds (kills -3 RPM graphs)
            minute_data = minute_data.dropna(subset=["grouped_speed"])
            minute_data = minute_data[
                (minute_data["grouped_speed"] >= VALID_RPM_MIN) &
                (minute_data["grouped_speed"] <= VALID_RPM_MAX)
            ]
    ...
    # Keep all minutes; require core metrics
    master_df = master_df.dropna(subset=["voltage_std", "motor_speed", "angle_mean", "grouped_speed"])

    # Ensure integer grouped speed for plotting
    master_df["grouped_speed"] = master_df["grouped_speed"].astype(int)
...
if __name__ == "__main__":
    ...
    metadata["time_max_min"] = parse_duration_minutes(metadata["dur"], default=60)
