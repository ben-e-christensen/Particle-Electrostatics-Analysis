...
def process_separate_graphs(parent_dir):
    ...
    for i, trial_path in enumerate(trial_folders):
        try:
            ...
            # Convert to numeric and FLIP CH2 ONLY
    ...
    if ch2_voltage_data_grouped:
        ...
        plt.title("CH2: Interpolated Peak Charge (Flipped)", fontsize=20, fontweight='bold')
    else:
    ...
    if ch3_voltage_data_grouped:
        ...
        plt.ylim(FIXED_VOLT_YMIN, FIXED_VOLT_YMAX)
        plt.title("CH3: Interpolated Peak Charge", fontsize=20, fontweight='bold')
    else:
