...
def generate_whole_run_summaries(df, materials, output_dir):
    ...
    for material in materials:
        ...
        plt.figure(figsize=(11, 8)) # Slightly larger figure for better spacing
...
def generate_hysteresis(df, materials, output_dir, y_col, y_label, filename_prefix):
    ...
    for material in materials:
        ...
        # Specific label check for Charge vs Angle
        final_y_label = "Std Dev Voltage (V)" if "Charge" in y_label or "voltage" in y_col else y_label
        ...
        plt.ylabel(final_y_label, fontweight='bold', labelpad=12)
...
def generate_physics_contours(df, speeds, output_dir):
    ...
    properties = [("density", "Density"), ("tribo_rank", "Tribo Rank"), ("resistivity", "Resistivity"), ("collapse_count", "Collapses")]
    for prop_col, prop_label in properties:
        ...
        for speed in speeds:
            ...
            try:
                ...
                plt.colorbar(cntr).set_label("Angle of Repose")
                plt.scatter(x, y, c=z, cmap="viridis", edgecolors='white', s=110, zorder=5)
            except: plt.close(); continue
            plt.xlabel(prop_label); plt.ylabel("Voltage Std Dev"); plt.savefig(os.path.join(prop_dir, f"RPM_{speed}.png")); plt.close()
...
def generate_4d_ridges(df, output_dir):
    ...
    for prop_col, prop_label in properties:
        ...
        try:
            ...
            eq_text = f"Ridge Eqs:\nCharge = {z_coef[0]:.3f}*RPM + {z_coef[1]:.2f}\n{prop_label} = {x_coef[0]:.3f}*RPM + {x_coef[1]:.2f}"
            ax.text2D(0.05, 0.95, eq_text, transform=ax.transAxes, fontsize=12, color='red', bbox=dict(facecolor='white', alpha=0.8))
        except: pass
        ...
        ax.set_xlabel(prop_label); ax.set_ylabel('Speed (RPM)'); ax.set_zlabel('Charge (V Std)'); plt.colorbar(img, ax=ax, shrink=0.6).set_label('Angle')
        plt.title(f"4D Ridge Analysis: {prop_label}"); plt.savefig(os.path.join(ridge_dir, f"Ridge_{prop_col}.png")); plt.close()
...
def process_run(rel_path, metadata):
    ...
    for material_folder in material_folders:
        ...
        for trial_folder in [f.path for f in os.scandir(material_folder) if f.is_dir()]:
            ...
            df["direction"] = "Increasing"; df.loc[max_idx:, "direction"] = "Decreasing"
            ...
            minute_data["collapse_count"] = minute_data["collapse_count"].fillna(0)
    ...
    print(f"Unified Analysis Complete. Graphs in: {output_dir}")
