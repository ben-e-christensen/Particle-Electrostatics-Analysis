...
# --- MECHANICAL-ISSUE EXCLUSION WINDOWS ---
# (start_hour, end_hour) in cumulative rel_time_hours. Rows inside these
# windows are dropped, and every later timestamp is shifted backward by the
# window's duration so the remaining data is squished together with no gap.
EXCLUSION_WINDOWS_HOURS = [
#     (27.1, 42.0),
#     (76.50, 86.0),
(0.0,0.00001)
]
...
def squish_time(times, exclusion_windows):
    """Given an array of (already-stitched) rel_time_hours values, returns
    (keep_mask, adjusted_times):
      - keep_mask: False for rows falling inside any exclusion window
      - adjusted_times: times with each exclusion window's duration removed,
        so everything after a window slides back to close the gap.

    Comparisons are always made against the ORIGINAL `times` array (not the
    running adjusted values), so multiple windows stack correctly regardless
    of which CSV file a row happens to live in.
    """
    times = np.asarray(times, dtype=float)
    keep_mask = np.ones(len(times), dtype=bool)
    adjusted = times.copy()

    for start, end in sorted(exclusion_windows):
        duration = end - start
        in_window = (times >= start) & (times <= end)
        keep_mask &= ~in_window
        after_window = times > end
        adjusted[after_window] -= duration

    return keep_mask, adjusted
    ...
    for i in range(1, 8):
        ...
        for csv_file in csv_files:
            ...
            try:
                ...
                # NOTE: cumulative_ms bookkeeping stays based on the RAW
                # (un-squished) timeline, so file-to-file stitching across
                # the whole dataset stays consistent regardless of exclusions.
                start_ms = df["ms"].min()
                ...
                # --- SQUISH: drop excluded rows, shift everything after back ---
                keep_mask, adjusted_hours = squish_time(df["rel_time_hours"].values, EXCLUSION_WINDOWS_HOURS)
                df = df[keep_mask].copy()
                df["rel_time_hours"] = adjusted_hours[keep_mask]
