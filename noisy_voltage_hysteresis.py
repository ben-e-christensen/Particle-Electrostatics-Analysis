...
# Choose between: "NONE", "RMS", or "LOWPASS"
FILTER_MODE = "RMS" 

# RMS Settings
RMS_WINDOW_SIZE = 50  # Number of rows to average over (adjust based on noise frequency)

# Low-Pass Settings
LOWPASS_CUTOFF_HZ = 2.0  # Frequencies above this are muted
ESTIMATED_FS_HZ = 100    # Your data logger's sampling rate in Hz (adjust based on your ms interval)
...
def apply_lowpass(data, cutoff_hz, fs_hz, order=4):
    """Zero-phase Butterworth low-pass filter to strip high-frequency ringing."""
    nyq = 0.5 * fs_hz
...
def main(parent_dir):
    ...
    output_dir = os.path.join(normalized_path, f"Volume_Master_Results_{FILTER_MODE}")
    
    print(f"=== Starting Volume Grid Generation for: {material} (Filter: {FILTER_MODE}) ===")
    ...
    for root, dirs, files in os.walk(normalized_path):
        if "experiment_log.csv" in files:
            ...
            try:
                ...
                # ==========================================
                if FILTER_MODE == "RMS":
                    # Calculate rolling Root Mean Square
                    df["Filtered_Voltage"] = np.sqrt((df["Voltage (V)"]**2).rolling(window=RMS_WINDOW_SIZE, center=True).mean())                    # Fill the NaNs at the edges caused by the rolling window
                    df["Filtered_Voltage"] = df["Filtered_Voltage"].bfill().ffill()

                elif FILTER_MODE == "LOWPASS":
                    # filtfilt cannot handle NaNs, ensure the array is solid
                    clean_volts = df["Voltage (V)"].interpolate().fillna(0).values
                    df["Filtered_Voltage"] = apply_lowpass(clean_volts, LOWPASS_CUTOFF_HZ, ESTIMATED_FS_HZ)

                else: # "NONE"
                    df["Filtered_Voltage"] = df["Voltage (V)"]
                # ==========================================
                
                # 1. Save the raw data for Angle and Hysteresis calculations (using RAW voltage)
                all_raw_data.append(df[["Trial_ID", "Volume", "Time (min)", "Motor Speed (RPM)", "Direction", "Angle (deg)", "Voltage (V)"]])

                # 2. Extract ONLY the peaks for the Voltage Time Series using the FILTERED voltage
                times = df["Time (min)"].values
                speeds = df["motor_speed"].values 
                volts = df["Filtered_Voltage"].values
